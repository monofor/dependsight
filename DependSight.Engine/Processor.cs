    ...
    public class Processor : IProcessor
    {
        public async Task<List<PVersion>> Check(DependencyCheck value)
        {
            ...
            foreach (var source in value.Sources)
            {
                ...
                var latest = await metadataResource.GetVersions(value.PackageName, value.IncludePrerelease, false, cacheContext, nugetLogger, CancellationToken.None);

                if (latest == null || !latest.Any()) continue;

                latest.ToList().ForEach(version => versions.Add(new PVersion
                {
                    Source = source,
